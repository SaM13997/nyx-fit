# Nyx Fit — Onboarding flow screens

Status: the concept was implemented and verified in the design section on 2026-09-11.
As of 2026-09-12, `/onboarding` uses the same flow screens with real authentication and
...
The four screens (`src/components/onboarding/flow/screens/`) form one flow with real local
...
`OnboardingFlow.tsx` renders the same four flow screens as `OnboardingConceptBoard.tsx`.
...
- A full-viewport onboarding surface supplies the definite height required by the shared shell's
...
| [Color Bends](https://reactbits.dev/backgrounds/color-bends) | `three` WebGL shader | No new dependency allowed; the ambient background is now the user-supplied raster art in `public/onboarding/` rendered by `OnboardingBackground.tsx` (CSS washes and SVG ribbons served as the interim stand-in and were removed) |
...
## Tokens (`.theme-flow`, `src/styles.css`)
...
`OnboardingBackground.tsx` renders the art with `object-cover object-top` and a native CSS 34 s
...
- Decorative layers (`OnboardingBackground`, the backdrop blur/wash layer, artwork) are
...
- `src/styles.css` — `.theme-flow` tokens (also mapped onto the shadcn semantic tokens so the
...
- `src/components/onboarding/flow/config.ts` — copy, options, step labels (reuses the real
  `ExperienceLevel` union from `../config`).
- `flow/classes.ts` — type recipes.
- `.../OnboardingBackground.tsx` — per-screen background art, sharp rendering, loading hints, and
  the CSS ambient scale loop.
- `flow/ScreenShell.tsx` — background art layer, brand header, `Progress` stepper, scroll body,
  bottom action row.
- `.../artwork.tsx` — bars card, capsule, rings, profile-card illustration, level icon.
- `flow/screens/*.tsx` — the four screens. Interactive UI is composed from the
...
- `src/components/ui/field.tsx` — central adjustment only: the checked `FieldLabel` fill uses
  the scoped `--accent` token so the selected choice card reads lime in `.theme-flow`.
- `flow/OnboardingConceptBoard.tsx` — prototype stage, tabs, viewport toggle, flow state.
- `flow/GoogleColorMark.tsx` — the official four-colour Google mark used on the white save
...
- The ambient art is user-supplied; it is raster, so swapping the mapping is a one-line change
  in `OnboardingBackground.tsx` (`backgroundSrc`).
...
- The initial concept needed no new production tests. The live integration updates the
  existing suite for flow copy and Radix semantics and adds regression coverage for Back
...
- shadcn composition rebuild: customizations live in the components, not at the usage sites.
  ...
  were added and `.theme-flow`/`.dark` were not touched.
- The rebuild kept the verification surface: route and query params, both
  `data-testid="flow-stage"` wrappers, the exact button labels, the 700 ms simulated save
